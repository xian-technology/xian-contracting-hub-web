...
def test_app_registers_developer_leaderboard_route(app_module: dict[str, object]) -> None:
    assert DEVELOPER_LEADERBOARD_ROUTE.lstrip("/") in app_module["app"]._unevaluated_pages


def test_developer_leaderboard_page_renders_public_sections() -> None:
    rendered = developer_leaderboard_index().render()
