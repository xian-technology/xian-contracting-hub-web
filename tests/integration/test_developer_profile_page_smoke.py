...
def test_app_registers_developer_profile_route(app_module: dict[str, object]) -> None:
    assert DEVELOPER_PROFILE_ROUTE.lstrip("/") in app_module["app"]._unevaluated_pages


def test_developer_profile_page_renders_public_profile_sections() -> None:
    rendered = developer_profile_index().render()
    ...
    assert any('"data-testid":"developer-profile-page"' in prop for prop in rendered_props)
    assert any('"data-testid":"developer-profile-loading"' in prop for prop in rendered_props)
    assert "Developer identity" in rendered_text
    assert "Loading developer profile" in rendered_text
