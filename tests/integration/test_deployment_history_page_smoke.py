...
from contracting_hub.pages.deployment_history import index as deployment_history_index
...
def test_app_registers_deployment_history_route(app_module: dict[str, object]) -> None:
    assert DEPLOYMENT_HISTORY_ROUTE.lstrip("/") in app_module["app"]._unevaluated_pages


def test_deployment_history_page_renders_history_and_shortcuts() -> None:
    rendered = deployment_history_index().render()
