...
- When adding page-level loading or failure wrappers, keep each page `data-testid` unique in the live DOM; duplicating the same test id on both the outer wrapper and an inner loaded section causes Playwright strict-mode locator failures.
- Reflex `rx.set_focus("...")` works reliably for modal-style focus management only after the target has rendered; in a state handler, set the open flag, `yield` once, then return the focus event, and return a second `rx.set_focus(...)` from the close handler to restore keyboard focus to the trigger.
- Local reset helpers that target a temporary SQLite path should open an explicit `Session(sa.create_engine(...))` for reseeding instead of routing through `session_scope()` / `rx.Model.get_db_engine()`; Reflex can cache the previous engine and accidentally seed the wrong database after env overrides.
- Admin version-manager state tests can cover most route-load, preview, and save/publish branches by instantiating the state with `_reflex_internal_init=True`, wiring `RouterData`, and mocking `session_scope()` plus the service entrypoints; that pattern is enough to raise coverage without hitting the database or booting Reflex.
- The full release gate currently passes under the checked-in [`.venv`](/home/endogen/projects/contracting-hub/.venv) via direct [`.venv/bin/ruff`](/home/endogen/projects/contracting-hub/.venv/bin/ruff), [`.venv/bin/pytest`](/home/endogen/projects/contracting-hub/.venv/bin/pytest), and [`.venv/bin/reflex`](/home/endogen/projects/contracting-hub/.venv/bin/reflex) invocations even when `python` is missing from `PATH`; `reflex export --no-zip` still emits the documented duplicate `SitemapPlugin` warning, but the build succeeds afterward.
